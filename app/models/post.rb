...
class Post < ActiveRecord::Base
 ...
 find(:all, :conditions => ['lower(title) LIKE ? OR lower(description)  LIKE ? OR lower(trade_for)  LIKE ?', search_condition, search_condition, search_condition])  
 ...
 def self.search_trades(query1)
    if !query1.to_s.strip.blank?
     search_condition = "%" + query1.to_s.strip.downcase + "%"
...
def self.search_trade_for(query2)
    if !query2.to_s.strip.blank?
      search_condition = "%" + query2.to_s.strip.downcase + "%"
      find(:all, :conditions => ['lower(trade_for)LIKE ?',search_condition])  
